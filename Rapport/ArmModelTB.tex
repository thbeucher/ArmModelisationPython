...
All parameters of the arm are defined in the file \textit{setupArmParameters} and it is implemented in the class \textit{ArmParameters}.
We find in this class different functions:
\begin{itemize}
\item[\textit{readSetupFile}]: Reads the setup file.
\item[\textit{massMatrix}]: Initializes parameters used for the inertia matrix.
\item[\textit{BMatrix}]: Defines the damping matrix B.
...
\end{align*}
\end{itemize}
You can find all the arm parameters values in the \hyperref[ArmParamTable]{Table~\ref{ArmParamTable}} in Appendix~\ref{sec:params}.

\subsection{Muscles parameters}
\label{sec:muscle_parameters}
All the muscles parameters are defined in the file \textit{setupMusclesParameters} and it is implemented in the class \textit{MusclesParameters}.
We find in this class different functions:
\begin{itemize}
...
\end{pmatrix}\\
\end{align*}
\item[\textit{activationVectorInit}]: Initializes the muscular activation vector.
\item[\textit{activationVectorUse}]: Builds the muscular activation vector from its 6 components.
\end{itemize}
You can find all the muscles parameters values in \hyperref[MuscleParamTable]{Table 3} in Appendix~\ref{sec:params}.
...
The equation~\ref{eq:rbd} is implemented in the class \textit{ArmDynamics} line 57:
\begin{lstlisting}
ddotq = np.dot(Minv,(Gamma - C - np.dot(armP.B, dotq)))
\end{lstlisting}
where \textit{np} refere to the numpy library in python.
We also find in this class all elements of the equation~\ref{eq:rbd}:
\begin{lstlisting}
#Inertia matrix
