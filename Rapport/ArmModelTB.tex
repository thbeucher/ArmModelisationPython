...
To recreate the delayed feedback about the position of the end effector we use an Unscented Kalaman Filter (UKF). We have not used a normal Kalman filter because we have to deal with a nonlinear system.
So the UKF allow to estimates the state of our system at the time t+1 given observations from times [0,t].
We feed the UKF with the real state but with a delay so the estimation is skew and the end-effector will reach the target before that the controller knowing. So the controller must decreases the velocity of the end-effector at the end of the trajectory in order to gain on accuracy.

\chapter{Conclusion}
\section{Difficulties experienced}
First, I had to learn a new programming language (python) in order to implement the model.
Then, I faced a mathematical problem when I haved to implement the RBFN algorithm for a multidimensional system.
Finally, I can say that the systemic difficulty that I experienced is about the choice on values for the different variables which change materially the model results.
...
I learn the programming language Python.
I also learned to use a computer cluster which allow me to do multiprocessing and multithreading in order to decrease the computing time.

\section{The continuation of the project}
After obtaining a satisfactory controller for each target sizes, we must generates statistical data in order to publish the results given by the model comparing to those obtained by our Slovenian colleagues Jan babic and Luka Peternel on humans.

At this time, we do not know yet if the model well describe the brain strategies.
...
virtual wall at a distance equal to 95% of the subject's arm length
...
located at the distance equal to 35% of the arm length from the virtual
wall and included 5 initial areas while the other line was located at
the distance equal to 55% of the arm length from the virtual wall and
